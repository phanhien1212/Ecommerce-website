...
const ProductHidden = () => {
  ...
  const [productHiddens, setProductHiddens] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const productHiddens = result.data.filter(
        (product) => product.status === 2
      );

      setProductHiddens(productHiddens);
      setInitialProducts(productHiddens);
  ...
  const handleFilter = (event) => {
    ...
    const filteredProducts = productHiddens.filter((product) =>
      product.name.includes(inputfilter)
    );
    setProductHiddens(filteredProducts);
  };
  //Phân Trang
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 10; // Số lượng mục trên mỗi trang
  const pageCount = Math.ceil(productHiddens.length / itemsPerPage);
  const handlePageClick = (pageNumber) => {
    setCurrentPage(pageNumber);
  };
  const startIndex = currentPage * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = productHiddens.slice(startIndex, endIndex);
  //EndPhaTrang
  const resetFilter = () => {
    // Reset products to initial list
    setProductHiddens(initialProducts);
        ...
        <div className="col-md-12 border rounded-3 mt-4">
          <div className="col-md-12 border-bottom">
            <div className="d-flex flex-row mx-auto">
              ...
              <div className="p-2 me-3 my-auto">
                <Link
                  to="/seller/productseller/producthidden"
          ...
          <div className="col-md-12 mt-3">
            <div className="row">
              <h5 className="col-md-6 mt-2">
                <span>{productHiddens.length}</span> Sản Phẩm
              </h5>
            </div>
          </div>
