...
const ProductViolate = () => {
  const [products, setProducts] = useState([]);
  const sellerId = localStorage.getItem("userId");
  const [user, setUser] = useState({});
  const [categories, setCategories] = useState([]);

  const [productViolates, setProductViolates] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const productViolates = result.data.filter(
        (product) => product.status === 3
      );
console.log(productViolates)
      setProductViolates(productViolates);
      setInitialProducts(productViolates);
      // Đếm số lượng sản phẩm có trạng thái là 1 (đã được duyệt)
  ...
  const handleFilter = (event) => {
    ...
    const filteredProducts = productViolates.filter((product) =>
      product.name.includes(inputfilter)
    );
    setProductViolates(filteredProducts);
  };
  //Phân Trang
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 10; // Số lượng mục trên mỗi trang
  const pageCount = Math.ceil(productViolates.length / itemsPerPage);
  const handlePageClick = (pageNumber) => {
    setCurrentPage(pageNumber);
  };
  const startIndex = currentPage * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = productViolates.slice(startIndex, endIndex);
  //EndPhaTrang
  const resetFilter = () => {
    // Reset products to initial list
    setProductViolates(initialProducts);
        ...
        <div className="col-md-12 border rounded-3 mt-4">
          <div className="col-md-12 border-bottom">
            <div className="d-flex flex-row mx-auto">
              ...
              <div className="p-2 me-3 my-auto">
                <Link
                  to="/seller/productseller/productoutstock"
              ...
              <div className="p-2 me-3 my-auto">
                <Link to="/seller/productseller/productviolate"
                 className="text-danger fw-bold">Vi phạm</Link>
              </div>
              <div className="p-2 me-3 my-auto">
          ...
          <div className="col-md-12 mt-3">
            <div className="row">
              <h5 className="col-md-6 mt-2">
                <span>{productViolates.length}</span> Sản Phẩm
              </h5>
            </div>
          </div>
