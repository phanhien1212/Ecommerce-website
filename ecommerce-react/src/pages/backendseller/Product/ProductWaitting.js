...
const ProductWaitting = () => {
  const [products, setProducts] = useState([]);
  const sellerId = localStorage.getItem("userId");
  const [user, setUser] = useState({});
  const [categories, setCategories] = useState([]);

  const [productWaittings, setProductWaittings] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const productWaittings = result.data.filter(
        (product) => product.status === 0
      );

      setProductWaittings(productWaittings);
      setInitialProducts(productWaittings);
  ...
  const handleFilter = (event) => {
    ...
    const filteredProducts = productWaittings.filter((product) =>
      product.name.includes(inputfilter)
    );
    setProductWaittings(filteredProducts);
  };
  //Phân Trang
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 10; // Số lượng mục trên mỗi trang
  const pageCount = Math.ceil(productWaittings.length / itemsPerPage);
  const handlePageClick = (pageNumber) => {
    setCurrentPage(pageNumber);
  };
  const startIndex = currentPage * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = productWaittings.slice(startIndex, endIndex);
  //EndPhaTrang
  const resetFilter = () => {
    // Reset products to initial list
    setProductWaittings(initialProducts);
          ...
          <div className="col-md-12 mt-3">
            <div className="row">
              <h5 className="col-md-6 mt-2">
                <span>{productWaittings.length}</span> Sản Phẩm
              </h5>
            </div>
          </div>
...
export default ProductWaitting;
