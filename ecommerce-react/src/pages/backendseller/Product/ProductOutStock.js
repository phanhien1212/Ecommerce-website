...
const ProductOutStock = () => {
  ...
  const [productOutStocks, setProductOutStocks] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const productOutStocks = result.data.filter(
        (product) => product.stockquantity === 0
      );

      setProductOutStocks(productOutStocks);
      setInitialProducts(productOutStocks);
  ...
  const handleFilter = (event) => {
    ...
    const filteredProducts = productOutStocks.filter((product) =>
      product.name.includes(inputfilter)
    );
    setProductOutStocks(filteredProducts);
  };
  //Phân Trang
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 10; // Số lượng mục trên mỗi trang
  const pageCount = Math.ceil(productOutStocks.length / itemsPerPage);
  const handlePageClick = (pageNumber) => {
    setCurrentPage(pageNumber);
  };
  const startIndex = currentPage * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = productOutStocks.slice(startIndex, endIndex);
  //EndPhaTrang
  const resetFilter = () => {
    // Reset products to initial list
    setProductOutStocks(initialProducts);
          ...
          <div className="col-md-12 mt-3">
            <div className="row">
              <h5 className="col-md-6 mt-2">
                <span>{productOutStocks.length}</span> Sản Phẩm
              </h5>
            </div>
          </div>
