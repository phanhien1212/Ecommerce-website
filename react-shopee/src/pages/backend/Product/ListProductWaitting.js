...
const ListProductWaitting = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const productwaitting = sortedProducts.filter((p) => p.status === 0);
  ...
  const handleConfirm = async (id, sellerid) => {
    try {
      ...
      if (result.status === 200) {
        ...
        toast.success(`Sản phẩm ${id} vừa được xác nhận!`);
      }
    } catch (error) {
...
export default ListProductWaitting;
