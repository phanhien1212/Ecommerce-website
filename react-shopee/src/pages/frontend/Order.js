...
const Order = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const filteredOrders = responseData.filter(
        (order) => order.user_id === parseInt(buyerId) && order.seller_id !== 6
      );
  ...
  const handleSaveFeedBack = async (order_id, product_id) => {
    ...
    try {
      ...
      if (resultProduct.status === 201) {
        const rating = resultProduct.data.rating;
        const score = rating >= 3 ? rating : rating === 2 ? -3 : -5;

        const addScore = new FormData();
        addScore.append("product_id", product_id);
        addScore.append("score", score);
  ...
  const handleCancel = async (order_id, seller_id) => {
    const updateStatus = new FormData();
    ...
    addNotification.append(
      "content",
      `Đơn hàng của bạn vừa bị hủy. XEM CHI TIẾT`
    );
    addNotification.append("title", `Bạn có 1 đơn hàng bị HỦY`);
    addNotification.append("recipient_id", seller_id);
    addNotification.append("status", 1);
    addNotification.append(
      "link",
      `/seller/orderseller/orderdetail/${order_id}`
    );
  ...
  const handleReturn = async (order_id, productid) => {
    const updateStatus = new FormData();
    updateStatus.append("status", 5);
    const resultFeedBack = await OrderService.updatestatusorderdetail(
      order_id,
      updateStatus
    );
